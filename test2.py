...
def test(args):
    ...
    obj_list = [c for c in obj_list if c != args.train_class]
    prompt_learner = AnomalyCLIP_PromptLearner(model.to("cpu"), AnomalyCLIP_parameters)
    checkpoint1 = torch.load(args.stage1_checkpoint_path)
    prompt_learner.load_state_dict(checkpoint1["prompt_learner"])

    checkpoint2 = torch.load(args.stage2_checkpoint_path)
    model.fusion.load_state_dict(checkpoint2["fusion"])
    model.visual_depth.load_state_dict(checkpoint2["visual_depth"])
...
############################################################ 考虑color
        ...
        whole_image2 = torch.cat([depth_image, image], dim = 0)
############################################################

        with torch.no_grad():
            prompts, tokenized_prompts, compound_prompts_text = prompt_learner(pointnet_input, cls_id = None)
            text_features = model.encode_text_learn(prompts, tokenized_prompts, compound_prompts_text).float()
            text_features = torch.stack(torch.chunk(text_features, dim = 0, chunks = 2), dim = 1)
            text_features = text_features/text_features.norm(dim=-1, keepdim=True)
            text_features = text_features.repeat_interleave(nv+1,dim=0) # 10,2,768

            image_features, patch_features = model.encode_image(whole_image, features_list, DPAM_layer = 20)
            image_features = image_features / image_features.norm(dim=-1, keepdim=True)

            image_features2, patch_features2 = model.encode_depth(whole_image2, args.features_list)
            ...
            text_probs = image_features.unsqueeze(1) @ text_features.permute(0, 2, 1) # (10,1,768)*(10,768,2)=(10,1,2)
            text_probs = (text_probs/0.07).softmax(-1) # 10,1,2
            text_probs = text_probs[:, 0, 1]           # 10
            # print('text_probs', text_probs.shape)
####################################################
            color_text_probs = text_probs[-b:]          # 1，rgb图的分数
            text_probs = text_probs[:-b]                # 9，渲染图的分数
            text_probs = torch.chunk(text_probs, nv, dim = 0)
            text_probs = torch.stack(text_probs, dim = 1).mean(1) # 1
            # print('text_probs', text_probs.shape)
####################################################
...
####################################################
            color_similarity_map = similarity_map[-b:]
            similarity_map = similarity_map[:-b]
####################################################
...
#####################################################################
            ...
            results[cls_name[0]]['color_pr_sp'].extend(color_anomaly_map.max() + color_text_probs.detach().cpu())
            results[cls_name[0]]['integrate_pr_sp'].extend((integrate_anomaly_map.max() + (color_text_probs + text_probs)/2).detach().cpu())
    ...
    color_table_ls = []
    color_image_auroc_list = []
    color_image_ap_list = []
    color_pixel_auroc_list = []
    color_pixel_aupro_list = []
    ...
    for obj in obj_list:
        ...
        if args.metrics == 'image-level':
            ...
            #### color
            color_image_auroc = image_level_metrics(results, obj, "image-auroc", modality = 'color_pr_sp')
            color_image_ap = image_level_metrics(results, obj, "image-ap", modality = 'color_pr_sp')
            color_table.append(str(np.round(color_image_auroc * 100, decimals=1)))
            color_table.append(str(np.round(color_image_ap * 100, decimals=1)))
            color_image_auroc_list.append(color_image_auroc)
            color_image_ap_list.append(color_image_ap) 
            #### point + color
            ...
            pixel_aupro = pixel_level_metrics(results, obj, "pixel-aupro", modality = 'anomaly_maps')
            table.append(str(np.round(pixel_auroc * 100, decimals=1)))
            table.append(str(np.round(pixel_aupro * 100, decimals=1)))
            pixel_auroc_list.append(pixel_auroc)
            pixel_aupro_list.append(pixel_aupro)
            #### color
            color_pixel_auroc = pixel_level_metrics(results, obj, "pixel-auroc", modality = 'color_anomaly_maps')
            color_pixel_aupro = pixel_level_metrics(results, obj, "pixel-aupro", modality = 'color_anomaly_maps')
            color_table.append(str(np.round(color_pixel_auroc * 100, decimals=1)))
            color_table.append(str(np.round(color_pixel_aupro * 100, decimals=1)))
            color_pixel_auroc_list.append(color_pixel_auroc)
            color_pixel_aupro_list.append(color_pixel_aupro)
            #### point + color
        ...
        color_table_ls.append(color_table)
    ...
    if args.metrics == 'image-level':
        ...
        color_table_ls.append(['mean', 
                        str(np.round(np.mean(color_image_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(color_image_ap_list) * 100, decimals=1))])

        color_results = tabulate(color_table_ls, headers=['objects', 'image_auroc', 'image_ap'], tablefmt="pipe")
        ...
        color_table_ls.append(['mean', str(np.round(np.mean(color_pixel_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(color_pixel_aupro_list) * 100, decimals=1)), 
                       ])

        color_results = tabulate(color_table_ls, headers=['objects', 'pixel_auroc', 'pixel_aupro'], tablefmt="pipe")
        ...
        color_results = tabulate(color_table_ls, headers=['objects', 'pixel_auroc', 'pixel_aupro', 'image_auroc', 'image_ap'], tablefmt="pipe")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--stage2_checkpoint_path", type=str, default='./checkpoint/', help='path to checkpoint')
    # model
