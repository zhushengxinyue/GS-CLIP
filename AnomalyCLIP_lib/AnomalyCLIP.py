...
from .fusion import LoRALayer, CoAttentionFusionBlock
import copy
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        if True:
            ...
            return self.ln_post(x_ori.permute(1, 0, 2)[:, 0, :]) @ self.proj, patch_tokens


        return x


def apply_lora_to_vit(vit_model: nn.Module, rank: int, alpha: int):
    for name, module in vit_model.named_modules():
        if isinstance(module, nn.Linear) and 'mlp' in name:
            parent_name = '.'.join(name.split('.')[:-1])
            layer_name = name.split('.')[-1]
            
            parent_module = vit_model.get_submodule(parent_name)
            
            lora_layer = LoRALayer(original_layer=module, rank=rank, alpha=alpha)
...
class AnomalyCLIP(nn.Module):
    def __init__(self,
        ...
        # fusion
        self.fusion = CoAttentionFusionBlock(d_model=768, nhead=4)
        self.visual_depth = copy.deepcopy(self.visual)
        for param in self.visual_depth.parameters():
            param.requires_grad = False
        apply_lora_to_vit(self.visual_depth, rank=8, alpha=8 * 2)
    ...
    def encode_depth(self, image, feature_list = [], ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        return self.visual_depth(image.type(self.dtype), feature_list, ori_patch = ori_patch, proj_use = proj_use, DPAM_layer = None, ffn = ffn)

    def fusion_r_d(self, render, depth):
        r = self.fusion(render, depth)
        return r
