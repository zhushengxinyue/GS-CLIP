...
def image_level_metrics(results, obj, metric, modality = 'pr_sp'):
    gt = results[obj]['gt_sp']
    pr = results[obj][modality]
    gt = np.array(gt)
    pr = np.array(pr)
...
def pixel_level_metrics(results, obj, metric, modality = 'anomaly_maps'):
